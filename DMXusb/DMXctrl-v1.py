#
#   DMXctrl-v1.py - DMX-Fader für USB 
#
#   20260711 - Jens & Gemini 3.5 Flash
#
...
# Ausgaben absichern für den .pyw Start ohne Konsole
#sys.stdout = open("dmx_output.log", "a")
#sys.stderr = open("dmx_error.log", "a")
...
def find_dmx_port():
    ...
    for port in ports:
        # Methode 1: Überprüfung der festen USB Hardware-IDs (VID und PID)
        # FTDI FT232R hat standardmäßig VID=0x0403 und PID=0x6001
        if port.vid == 0x0403 and port.pid == 0x6001:
            print(f"FTDI-Interface über Hardware-ID gefunden: {port.device}")
            return port.device
            
        # Methode 2 (Fallback): Falls Windows die IDs verschluckt, Textprüfung
        if port.description:
            if "FT232" in port.description or "FTDI" in port.description:
                print(f"FTDI-Interface über Textbeschreibung gefunden: {port.device} ({port.description})")
                return port.device
                
    return None

target_port = find_dmx_port()

try:
    if target_port:
        ser = serial.Serial(
            port=target_port,  # Benutzt automatisch den gefundenen Port (z.B. 'COM8')
            baudrate=250000,
        ...
        print("Kein FT232R-Interface gefunden.")
